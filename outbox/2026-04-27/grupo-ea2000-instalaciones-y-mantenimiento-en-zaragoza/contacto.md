# Contacto — Grupo EA2000

**Fecha de generación:** 2026-04-27

## Destinatario
- **Nombre:** ⚠ Identificar antes de enviar
- **Cargo:** Director Técnico / Director de Operaciones
- **Web:** grupoea2000.com
- **Zona:** Zaragoza

## Canal recomendado
1. **LinkedIn (preferente)** → connection sin nota. Búsqueda: `"EA2000" Zaragoza mantenimiento OR técnico`
2. **Email (respaldo)** → pendiente Apollo+Hunter enrichment.

## Qué enviar
- LinkedIn: `linkedin-paso1.md`
- Email: `email-t1.html` cuando se consiga dirección directa. Asunto: "Mantenimiento industrial y el contrato anual"

## Acciones pendientes antes de enviar
- Apollo+Hunter → email decisor.
- Identificar nombre concreto en LinkedIn.
- Sustituir "Hola," por "Hola {nombre},".
